...
void drawImage(GLuint file, float x, float y, float w, float h, float travalue)
{
	...
	if (loadingcount % 12 == 0) {
		for (int m = 0; m < loadingcount/12; m++) {
			base = base + "|";
		}
		char string[100000];
...
void render()
{
	...
	if (blink == false) {
		...
		renderbitmap(-50, -50, GLUT_BITMAP_HELVETICA_18, subtitle, 1, 1, 1);
	}
...
int main()
{
	...
	xRotated = 33;
	yRotated = 40;
	ifstream in1("casesu.txt");
	ifstream in2("deathu.txt");
