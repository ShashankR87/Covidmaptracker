...

{
	if (statecode != -1) {
		...
		int loopCnt = 0;
		for (int i = 0; i <= statecases + (statecases / 9); i += statecases / 9, loopCnt++) {
			casesScale[loopCnt] = i;

			//std::cout << "CasesScale " << i << '\n';
		}	//initializing y axis=value		9145/9
		std::cout << "loopcnt " << loopCnt << '\n';

		int deathsScale[50];
		int loopCnt1 = 0;
		for (float i = 0; i <= (float)statedeaths + (float)(statedeaths / 9.0); i += statedeaths / 9.0, loopCnt1++) {
			deathsScale[loopCnt1] = i;
