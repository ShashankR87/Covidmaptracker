...
void plotPoints() {
	...
	for (; i < cases.end(); i++, k++, j += 200 / (cnt), datecnt++) {	//draw the points cases
		...
		*/
		...
		while (indx2 == indx1)
			indx2 = rand() % 24;
		x1 = statemids[indx1][0];
		y1 = statemids[indx1][1];
		x2 = statemids[indx2][0];
		y2 = statemids[indx2][1];

		int dx, dy, ii, e, bx1, bx2, by1, by2;
...
void plotPointsState(int zzz) {
	...
	float k1=0;
	int datecnt = 0;
	for (; i < casesState[zzz].end(); k1+=(*k / 96.00),i1+=(*i/196.00),i++, k++, j += 200 / (cnt), datecnt++) {	//draw the points cases	
		
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_COLOR, GL_DST_COLOR);
		draw_casecircle(i1, 0, 0);//max radius 196
		glDisable(GL_BLEND);
		draw_deathcircle(k1, 0, 0);
...

{
	if (statecode != -1) {
		...
		int oldDeathVal = -1;
		for (int i = -450; i < -251; i += 20) {             //draw small horizontal lines+text which was initialized for deaths graph
			if (oldDeathVal != deathsScale[loopCnt1]) {
				glBegin(GL_LINES);
				glVertex3f(275, i, 10);
				glVertex3f(270, i, 10);
				glEnd();
